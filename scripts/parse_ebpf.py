...
Replaces merge_falco.py and YARA/semgrep static features.
Usage: python3 scripts/parse_ebpf.py <opensnoop.json> <tcpconnect.json> 
                                      <syscount.json> <decoy_log.json>
...
def parse_opensnoop(path: str) -> dict:
    counts = {
...
def parse_tcpconnect(path: str) -> dict:
...
def parse_syscount(path: str) -> dict:
...
def extract_patterns(opensnoop_path: str, syscount_path: str) -> dict:
    """
    Detect DySec-style behavioral patterns.
    Based on Table VI from DySec paper.
    """
    # Read all syscalls in order from opensnoop
    syscall_sequence = []
    try:
    ...
    # ENOENT count — malicious probing indicator
    enoent_count = sum(1 for s in syscall_sequence if "enoent" in s)

    return {
        # Pattern 4 — C2 communication
        "pattern_c2_communication":   has_sequence(
            ["socket", "bind", "listen", "accept", "execve"]),
        # Pattern 5 — Process injection
        "pattern_process_injection":  has_sequence(
            ["mmap", "fork", "ptrace", "execve"]),
        # Pattern 10 — Malicious file probing
        "pattern_malicious_probing":  enoent_count > 50,
        # Pattern — Privilege escalation
        "pattern_privilege_escalation": has_sequence(
            ["ioctl", "setresuid", "setresgid", "execve"]),
        # Pattern — Ransomware file locking
        "pattern_file_locking":       has_sequence(
            ["openat", "fstat", "fcntl"]),
    }
...
def main():
    if len(sys.argv) < 5:
        print("Usage: parse_ebpf.py <opensnoop> <tcpconnect> "
              "<syscount> <decoy_log>")
        sys.exit(1)
    ...
    opensnoop_features  = parse_opensnoop(opensnoop_path)
    tcpconnect_features = parse_tcpconnect(tcpconnect_path)
    syscount_features   = parse_syscount(syscount_path)
    pattern_features    = extract_patterns(opensnoop_path, syscount_path)
