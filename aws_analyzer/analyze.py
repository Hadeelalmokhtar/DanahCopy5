...
def analyze_package(pkg_name, run_number="0"):
    print(f"\n[analyzer] Starting eBPF analysis for: {pkg_name} (run={run_number})")
    ...
    mon_sys = None  # strace runs with the package directly
    time.sleep(2)
    ...
    # strace stops automatically when package finishes
    f_open.flush()
    f_tcp.flush()
    f_sys.flush()
    f_open.close()
